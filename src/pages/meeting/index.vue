...
import XYRTC, { StatusInfo } from '@xylink/xy-mp-sdk';
import {
  CustomTemplateInfo,
  IBulkRoster,
  LayoutInfo,
...
const roster = ref<RosterObj>(); // 会中roster信息
const layout = ref<LayoutInfo[]>([]); // 布局对象
...
const layoutMode = ref<LayoutMode>(LayoutMode.AUTO);
...
const newLayout = computed(() => {
  const temp: NewLayout[] = layout.value.map((item: LayoutInfo) => {
...
onMounted(async () => {
  ...
  const response = await XYClient.value?.makeCall({
    ...
    password,
    displayName,
    isAudioMute: audioMute.value,
    isVideoMute: videoMute.value,
  });

  if (response) onGetCallStatus(response);
});
 ...
 */
const onGetCallStatus = (response: StatusInfo) => {
 ...
 *
 */
const onFullScreenContent = (item: LayoutInfo, event: any) => {
  Event.click(event, () => {
 ...
 * 自定义布局，处理模板计算
 */
const handleCustomLayout = (detail: RosterObj) => {
  // 处理custom模式自定义布局
  const newDetails: CustomTemplateInfo[] = [];
  // 获取roster数据
  ...
  const { HD, NORMAL } = QualityType;

  const selfDetailObj: CustomTemplateInfo = {
